...
void CountPPQN()
{
  ...
  if (ppqn % (pattern[ptrnBuffer].scale / 2) == 0)
    blinkFast = LOW;
  if (ppqn % 4 == 0)
    MIDI.sendRealTime(Clock); //MidiSend(CLOCK_CMD);//as NAVA seq is 96ppqn we need to send clock each 4 internal ppqn
  ...
  if (isRunning)
  {
    if (ppqn % pattern[ptrnBuffer].scale == (pattern[ptrnBuffer].scale / 2))
      tapStepCount++;

    if (tapStepCount > pattern[ptrnBuffer].length)
      tapStepCount = 0;

    if (ppqn % pattern[ptrnBuffer].scale == 0)
      stepChanged = TRUE; //Step changed ?
    ...
    {                      //Each Step
      ...
      case BACKWARD:
        ...
        break;
      case PING_PONG:
        if (curStep == pattern[ptrnBuffer].length && changeDir == 1)
          changeDir = 0;
        else if (curStep == 0 && changeDir == 0)
          changeDir = 1;
        if (changeDir)
          curStep = stepCount;
        else
      ...
      if (bitRead(pattern[ptrnBuffer].inst[CH], curStep))
        tempDoutTrig = B10; //CH trig
      if (bitRead(pattern[ptrnBuffer].inst[OH], curStep))
        tempDoutTrig = 0; // OH trig
      SetDoutTrig(tempDoutTrig);
      TRIG_HIGH;
      //ResetDoutTrig();
      stepCount++;
    }
    if (stepCount > pattern[ptrnBuffer].length)
    { // && (ppqn % 24 == pattern[ptrnBuffer].scale - 1))
      endMeasure = TRUE;
