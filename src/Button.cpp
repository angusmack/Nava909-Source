...
void ButtonGet()
{
  ...
  if (enterBtn.pressed)
  {
    if (millis() - enterBtn.curTime > HOLD_TIME)
      enterBtn.hold = HIGH;
  }

  //init step button------------------------------------------------
  for (byte a = 0; a < NBR_STEP_BTN; a++)
  { //a = step button number
    ...
    if (stepBtn[a].curState != stepBtn[a].prevState)
    {
      if ((stepBtn[a].pressed == LOW) && (stepBtn[a].curState == HIGH))
      {
        ...
        else
        {
          ...
          {
            //if vel > MID_VEL
            if (pattern[ptrnBuffer].velocity[curInst][a] > instVelLow[curInst])
              stepBtn[a].counter = 3;
            else
              stepBtn[a].counter = 2;
          }
