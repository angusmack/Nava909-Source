...
void EncGet()
{
  ...
  if (shufBtn.pressed && curSeqMode == PTRN_STEP)
  {
    ...
    if (pattern[ptrnBuffer].shuffle <= 1)
      pattern[ptrnBuffer].shuffle = 1;
    if (pattern[ptrnBuffer].shuffle >= MAX_SHUF_TYPE)
      pattern[ptrnBuffer].shuffle = MAX_SHUF_TYPE;
  ...
  else
  {
    ...
    if (seq.bpm != curBpm)
    {
      curBpm = seq.bpm;
      TimerSetFrequency();
      if (curSeqMode != PTRN_STEP || tempoBtn.pressed)
        needLcdUpdate = TRUE;
    }
  }
}
