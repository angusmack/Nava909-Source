...
void KeyboardUpdate()
{
  ...
  if (keyboardMode)
  {
    ...
    {
      keybOct--;
      if (keybOct < 0 || keybOct > MAX_OCT - 1)
        keybOct = 0;
      needLcdUpdate = TRUE;
    }
    ...
    {
      noteIndex--;
      if (noteIndex < 0 || noteIndex > MAX_EXT_INST_NOTE - 1)
        noteIndex = 0;
      MidiSendNoteOn(seq.TXchannel, pattern[ptrnBuffer].extNote[noteIndex], HIGH_VEL);
      needLcdUpdate = TRUE;
    }
    if (backBtn.justRelease)
      MidiSendNoteOff(seq.TXchannel, pattern[ptrnBuffer].extNote[noteIndex]);

    if (fwdBtn.justPressed)
    {
      noteIndex++;
      if (noteIndex >= MAX_EXT_INST_NOTE)
        noteIndex = MAX_EXT_INST_NOTE;
      MidiSendNoteOn(seq.TXchannel, pattern[ptrnBuffer].extNote[noteIndex], HIGH_VEL);
      needLcdUpdate = TRUE;
    }
    if (fwdBtn.justRelease)
      MidiSendNoteOff(seq.TXchannel, pattern[ptrnBuffer].extNote[noteIndex]);
      ...
      for (byte a = 0; a < NBR_STEP_BTN; a++)
      {
        ...
        if (stepBtn[a].curState != stepBtn[a].prevState)
        {
          if ((stepBtn[a].pressed == LOW) && (stepBtn[a].curState == HIGH))
          {
            ...
            if (pattern[ptrnBuffer].extLength > MAX_EXT_INST_NOTE)
