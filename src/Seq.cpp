...
void SeqParameter()
{
  ...
  if (curSeqMode == PTRN_STEP || curSeqMode == PTRN_TAP)
  {
    ...
    if (readButtonState == 0)
      doublePush = 0; //init double push if all step button  released
    ...
    if (clearBtn && !keyboardMode && curSeqMode != PTRN_TAP)
    {
      ...
      if (isRunning)
      {
        ...
        if (curInst == CH)
          pattern[ptrnBuffer].velocity[CH][curStep] = instVelHigh[HH]; //update HH velocity that OH is trigged correctly
      }
      else
      { //clear full pattern
    ...
    //-------------------shuffle Button------------------------------
    if (shufBtn.justPressed || shufBtn.justRelease)
      needLcdUpdate = TRUE;
    ...
    if (curSeqMode == PTRN_STEP)
    {
      ...
      if (trackPosNeedIncremante && group.length)
      { //&& stepCount > 0)
        group.pos++;
        if (group.pos > group.length)
          group.pos = 0;
    ...
    else if (curSeqMode == PTRN_TAP)
    {
      if (clearBtn)
      {
        bitClear(pattern[ptrnBuffer].inst[curInst], curStep);
        if (curInst == CH)
          pattern[ptrnBuffer].velocity[CH][curStep] = HIGH_VEL; //update HH velocity that OH is trigged correctly
        patternWasEdited = TRUE;
      }
        ...
        for (byte a = 0; a < NBR_STEP_BTN; a++)
        {
          ...
          if (stepBtn[a].curState != stepBtn[a].prevState)
          {
            if ((stepBtn[a].pressed == LOW) && (stepBtn[a].curState == HIGH))
            {
              ...
              case BD_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelHigh[BD];
                tempVel = instVelHigh[BD];
                break;
              case BD_LOW_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelLow[BD];
                tempVel = instVelLow[BD];
                break;
              case SD_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelHigh[SD];
                tempVel = instVelHigh[SD];
                break;
              case SD_LOW_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelLow[SD];
                tempVel = instVelLow[SD];
                break;
              case LT_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelHigh[LT];
                tempVel = instVelHigh[LT];
                break;
              case LT_LOW_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelLow[LT];
                tempVel = instVelLow[LT];
                break;
              case MT_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelHigh[MT];
                tempVel = instVelHigh[MT];
                break;
              case MT_LOW_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelLow[MT];
                tempVel = instVelLow[MT];
                break;
              case HT_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelHigh[HT];
                tempVel = instVelHigh[HT];
                break;
              case HT_LOW_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelLow[HT];
              ...
              case CH_BTN:
                ...
                {
                  if (isRunning)
                    pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelHigh[CH];
              ...
              case CH_LOW_BTN:
                ...
                {
                  if (isRunning)
                    pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelLow[CH];
                  tempVel = instVelLow[CH];
                }
                break;
              case RM_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelHigh[RM];
                tempVel = instVelHigh[RM];
                break;
              case HC_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelHigh[HC];
                tempVel = instVelHigh[HC];
                break;
              case CRASH_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelHigh[CRASH];
                tempVel = instVelHigh[CRASH];
                break;
              case RIDE_BTN:
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelHigh[RIDE];
              ...
              {
                if (isRunning)
                  pattern[ptrnBuffer].velocity[instOut[a]][tapStepCount] = instVelHigh[OH];
              ...
              else
              {
                ...
                SetDoutTrig(0);
                if (isRunning)
                  bitSet(tempInst[instOut[a]], tapStepCount);
              }
            }
          }
      ...
      if (endMeasure)
      { //Update pattern at the end of measure to not get a double trig
  ...
  if (curSeqMode == TRACK_WRITE)
  {
    ...
    {
      trk.pos--;
      if (trk.pos < 0 || trk.pos > MAX_PTRN_TRACK)
        trk.pos = 0;
    ...
    {
      trk.pos++;
      if (trk.pos > MAX_PTRN_TRACK)
        trk.pos = MAX_PTRN_TRACK;
      nextPattern = track[trkBuffer].patternNbr[trk.pos];
      if (curPattern != nextPattern)
        selectedPatternChanged = TRUE;
      needLcdUpdate = TRUE;
  ...
  if (curSeqMode == TRACK_PLAY)
  {
    ...
    { //(endMeasure)
      trk.pos++;
      if (trk.pos >= track[trkBuffer].length)
        trk.pos = 0;
      nextPattern = track[trkBuffer].patternNbr[trk.pos];
      if (curPattern != nextPattern)
        selectedPatternChanged = TRUE;
